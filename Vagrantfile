...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "buster", autostart: false do |distro|
    if SALT_VERSION.to_f < 2019
      raise StandardError.new "Salt version < 2019 not available for release 'buster'"
    end
  ...
  SHELL_SCRIPT = <<SCRIPT
echo "Installing Salt version #{SALT_VERSION} (Python#{PYTHON_VERSION})"
OSCODENAME=$(lsb_release -cs)
DISTRO=$(lsb_release -is);
DISTRO=${DISTRO,,}
VARIANT=$(lsb_release -rs)
apt-get install apt-transport-https
APT_URL="https://repo.saltstack.com/#{REPO_SUBPATH}/${DISTRO}/${VARIANT/\.*/}/amd64/#{SALT_VERSION}"
